...
def get_model_transfer_learning(model_name="resnet18", n_classes=50):
    ...
    if hasattr(models, model_name):
        ...
        raise ValueError(f"Model {model_name} is not known. List of available models: "f"https://pytorch.org/vision/{torchvision_major_minor}/models.html")

    # Freeze all parameters in the model
    # HINT: loop over all parameters. If "param" is one parameter, "param.requires_grad = False" freezes it
    for p in model_transfer.parameters():
        if p.requires_grad:
            p.requires_grad = False

    # Add the linear layer at the end with the appropriate number of classes
    num_ftrs  = model_transfer.fc.in_features # 1. get numbers of features extracted by the backbone
    model_transfer.fc  = nn.Linear(num_ftrs, n_classes) # 2. Create a new linear layer with the appropriate number of inputs and outputs
